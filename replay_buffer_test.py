...
class ReplayBuffer(object):
    def __init__(self, size, dsize, n_emu=1):
        ...
        """
        # One buffer for each emulator + 1 buffer for demo data
        self._storage = [0] * size #[[] for _ in range(n_emu + 1)]
        self._maxsize = size #round(size / n_emu) * n_emu
        self._next_idx = 0 #[0 for _ in range(n_emu + 1)]
    ...
    def add(self, obs_t, action, reward, obs_tp1, done):
        ...
        self._next_idx += 2 #(self._next_idx + 1) % self._maxsize

    def add_new(self, obs_t, action, reward, obs_tp1, done, emu_id=1, demo=False):
        data = (obs_t, action, reward, obs_tp1, done, self._traj_id)
        ...
        if self._next_idx[emu_id] >= len(self._storage[emu_id]):
            self._storage[emu_id].append(data)
        else:
            self._storage[emu_id][self._next_idx[emu_id]] = data

        if demo:
            self._next_idx[emu_id] = (self._next_idx[emu_id] + 1) % self._demosize
        else:
            self._next_idx[emu_id] = (self._next_idx[emu_id] + 1) % self._maxsize
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def _sample_proportional(self, batch_size, n_step=0):
        ...
        for _ in range(batch_size):
            ...
            print("Lenght ", len(self._storage), self._it_sum.sum(0, len(self._storage) - n_step - 1))
            print("Max ", self._maxsize, self._it_sum.sum(0, self._maxsize - n_step - 1))

            mass = random.random() * self._it_sum.sum(0, self._maxsize - n_step - 1)
    ...
    def sample(self, batch_size, beta):
        ...
        max_weight = (p_min * self._maxsize) ** (-beta)

        for idx in idxes:
            p_sample = self._it_sum[idx] / self._it_sum.sum()
            weight = (p_sample * self._maxsize) ** (-beta)
...
if __name__ == '__main__':
    replay_buffer = PrioritizedReplayBuffer(20, 0, alpha=0.6)
    for i in range(1, 5):
