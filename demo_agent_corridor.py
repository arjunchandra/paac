...
def plot_value_function(sess, q_max, v_func, input_ph, env):
            ...
            for y in range(0, grid_shape[1]):
                    if env.desc[x,y] != b'H':
                        ...
                        try:
                            ...
                            plt.text(y+0.6, grid_shape[0]-0.5-x, i, fontweight='bold', horizontalalignment='center', verticalalignment='top')
                        except ValueError:
                            pass
    ...
    for x in range(0, grid_shape[0]):
            for y in range(0, grid_shape[1]):
                if env.desc[x,y] != b'H':
                    plt.text(y+0.5, grid_shape[0]-0.8-x, env.desc[x,y].decode('utf-8'), fontweight='bold', horizontalalignment='center', verticalalignment='top')
...
def train_agent(): 
    ...
    env_id = 9
    env = GymEnvironment(envs[env_id][0], env_class = envs[env_id][1], visualize = False, agent_history_length=1)
    env2 = GymEnvironment(envs[env_id][0], env_class = envs[env_id][1], visualize = False, agent_history_length=1)
    num_actions = env.env.action_space.n
    input_shape = list(env.env.observation_space.shape)
    ...
    with tf.Session() as sess:
        ...
        for epi in range(1, num_episodes+1):
            ...
            j = 0
            #The Q-Network
            while (j < max_epLength) and (not d): #If the agent takes longer than 100 moves to reach either of the blocks, end the trial.
                j+=1
            ...
            if epi % k == 0:
                ...
                print("Episodes: {}, Steps: {}, Epi. succ. rate: {}%, Avg. rew/epi:{:.2f}%, Acc. reward: {:.2f}, Epsilon: {:.2f}".format(epi, total_steps, succ_epi*100.0/k, np.mean(rList)*100, acc_rew, e))
                rList = []
                succ_epi = 0
